...
from tern_svd.dist import sync_tern_svd_layer, dispatch
...
def parse_args():
    ...
    parser.add_argument(
        "--bin_svd_finetune",
        action="store_true",
        help="finetune with bin svd",
    )
    parser.add_argument(
        "--bin_svd_cos_theta", type=float, default=0.8386, help="cos theta of bin svd."
    )
    parser.add_argument(
        "--bin_svd_eta", type=float, default=float('Inf'), help="eta of bin svd."
    )
...
def main():
    ...
    accelerator = Accelerator(gradient_accumulation_steps=args.gradient_accumulation_steps, kwargs_handlers=[DistributedDataParallelKwargs(broadcast_buffers=False)], **accelerator_log_kwargs)
    ...
    if 'timm' in args.model_name_or_path:
        import timm
        from timm.data import resolve_data_config
        from timm.data.transforms_factory import create_transform
        from transformers.modeling_outputs import ImageClassifierOutputWithNoAttention

        class pixel_values_wrapper(torch.nn.Module):
            ...
            def forward(self, *args, **kwargs):
                ...
                return ImageClassifierOutputWithNoAttention(logits=logits)


        with replace_Linear_to_ternary_SVD_linear():
            model = timm.create_model('hf-hub:'+args.model_name_or_path, pretrained=True)
        train_transforms = create_transform(is_training=True, **resolve_data_config(model.pretrained_cfg, model=model))
        val_transforms = create_transform(is_training=False, **resolve_data_config(model.pretrained_cfg, model=model))
        model = pixel_values_wrapper(model)
    ...
    if args.bin_svd_finetune and args.bin_svd_tol > 0:
        trans_fun, s_fun = transform_policy_for_QAT(steps=20, tolerance=args.bin_svd_tol, verbose=False, cos_thresh=args.bin_svd_cos_theta)
        ...
        def bin_svd():
            ...
            counts_i = dispatch(torch.distributed.get_world_size())

            @tern_svd_layer_patch
            def _trans(M):
                num = get_num(M)
                if counts_i.get_rank(num) == torch.distributed.get_rank():
                    M.weight_to_usv(trans_fun, s_fun, prune_rate=args.bin_svd_eta)

            counts_j = dispatch(torch.distributed.get_world_size())

            @tern_svd_layer_patch
            def _sync(M):
                num = get_num(M)
                src = counts_j.get_rank(num)
                sync_tern_svd_layer(M, src)
    ...
    if args.num_train_epochs == 0:
        ...
        if args.bin_svd_tol > 0:
            trans_fun = transform_policy(steps=20, tolerance=args.bin_svd_tol, verbose=True, cos_thresh=0.8386)
        ...
        logger.info(f"epoch {-1}: {eval_metric}")
